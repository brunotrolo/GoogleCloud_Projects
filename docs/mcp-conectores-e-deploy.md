...
## 🎨 Ícone do conector (favicon)

O claude.ai **não** tem campo para escolher o ícone de um conector personalizado:
ele deriva o ícone do **favicon servido no host da URL** do conector. Por isso cada
servidor agora serve um favicon próprio (SVG) e uma página `/` que o referencia:

| MCP | Ícone servido | Rotas |
|-----|---------------|-------|
| WhatsApp | logo do WhatsApp (balão verde) | `GET /favicon.svg`, `/favicon.ico`, `/` |
| OpLab | ferramenta de análise (gráfico de candles) | idem |
| Cockpit | logo do Google Sheets (planilha verde) | idem |

O SVG fica embutido no próprio `index` de cada servidor (constante `FAVICON_SVG`),
sem arquivo de asset extra — o deploy não muda.

> **Para o ícone aparecer:** o claude.ai faz cache do favicon. Se o conector já
> existia, pode ser preciso **remover e re-adicionar** o conector (ou aguardar o
> cache expirar) para ele buscar o novo ícone. Conectores novos já pegam na hora.
