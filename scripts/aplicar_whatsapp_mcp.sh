...
# O repo é clonado como root e depois tem o dono trocado para whatsapp-mcp; sem isto,
# o `git pull` (rodando como root) falha com "dubious ownership" e o set -e aborta o
# startup-script ANTES do npm install/restart — deixando o código velho.
# IMPORTANTE: usar --system (grava em /etc/gitconfig, sempre lido) — o --global
# depende de $HOME=/root, que NÃO vale no contexto do startup-script do boot; por
# isso os git pull do boot vinham falhando em silêncio. O -c inline é reforço.
git config --system --add safe.directory /opt/whatsapp-mcp || true
if [ -d /opt/whatsapp-mcp/.git ]; then
  git -C /opt/whatsapp-mcp -c safe.directory=/opt/whatsapp-mcp pull -q
else
...
if [ -n "$MCP_API_KEY" ]; then
  ...
  echo "  URL p/ conector claude.ai:  https://$HOSTNAME/mcp/$MCP_API_KEY"
else
  echo "  X-API-Key:            (preservada — leia com:"
  echo "                         sudo grep MCP_API_KEY /etc/systemd/system/whatsapp-mcp.env  na VM)"
  echo "  URL p/ conector claude.ai:  https://$HOSTNAME/mcp/<SUA_CHAVE>"
fi
echo "  Health (público):     https://$HOSTNAME/health"
echo "═══════════════════════════════════════════════════"
echo ""
echo "PRÓXIMO PASSO (só na 1ª vez OU se trocar o número remetente) — parear via QR:"
echo "  gcloud compute ssh $VM_NAME --project=$PROJECT_ID --zone=$ZONE"
echo "  sudo journalctl -u whatsapp-mcp -f"
echo "  (escaneie o QR com o celular REMETENTE: WhatsApp → Aparelhos conectados → Conectar)"
echo ""
echo "No claude.ai: Conectores → Adicionar conector personalizado → cole a URL acima"
echo "(com a chave no final). Não precisa de OAuth. Ver docs/whatsapp-mcp-arquitetura.md."
